...
if file is None:
    ...
    if class_names[np.argmax(predictions)] == 'trash':
        ...
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling trash waste'
            summarized_text =generate_content("gemini-pro", content)
            text_to_audio(summarized_text)
            image_urls = []    
            if title:
                for i in range(1, 31, 10):
                    image_urls.extend(concat_google_images(title, i))
            if len(image_urls) < num_images:
                    st.warning("")
                    additional_images = bing_image_search(title, num_images - len(image_urls))
            ...
            for i, url in enumerate(image_urls):
                try:
                    ...
                    if response.status_code == 200:
                        image_file_path = f"{image_folder}/image_{i}.jpg"
                        with open(image_file_path, 'wb') as file:
                            file.write(response.content)
                        image_files.append(image_file_path)
                    else:
                        st.warning(f"Failed to fetch image from {url}. Status code: {response.status_code}")
                except (ConnectionError, ReadTimeout) as e:
                    st.warning(f"Failed to download image from {url}. Error: {str(e)}")
                    break
                except Exception as e:
                    st.warning(f"An unexpected error occurred while downloading image from {url}. Error: {str(e)}")
                    break
            if image_files:
                ...
                for image_file in image_files:
                    if os.path.exists(image_file):
                        try:
                            clip = ImageClip(image_file).set_duration(5).resize((1280, 720))
                            clips.append(clip)
                        except Exception as e:
                            st.warning(f"Failed to process {image_file}. Error: {str(e)}. Skipping this image.")
                    else:
                        st.warning(f"Image file {image_file} does not exist. Skipping this image.")
                final_clip = concatenate_videoclips(clips, method="compose")
                final_clip.write_videofile("output.mp4", fps=24)
                st.success("Video generated successfully")
                final_clip = final_clip.set_audio(AudioFileClip("output.mp3"))
                final_clip.write_videofile("output_with_audio.mp4", codec="libx264", audio_codec="aac", fps=24)
                st.markdown(get_binary_file_downloader_html("output_with_audio.mp4", "Download Video"), unsafe_allow_html=True)
            else:
                st.warning("No valid images found for the given title.")
    ...
    if class_names[np.argmax(predictions)]== 'plastic':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling plastic waste'
    ...
    if class_names[np.argmax(predictions)] == 'paper':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling paper waste'
    ...
    if class_names[np.argmax(predictions)] == 'metal':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling metal waste'
    ...
    if class_names[np.argmax(predictions)] == 'cardboard':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling cardboard waste'
    ...
    if class_names[np.argmax(predictions)] == 'shoes':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling shoes waste'
    ...
    if class_names[np.argmax(predictions)] == 'clothes':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling clothes waste'
    ...
    if class_names[np.argmax(predictions)] == 'battery':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling battery waste'
    ...
    if class_names[np.argmax(predictions)] == 'biological':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling biological waste'
    ...
    if class_names[np.argmax(predictions)] == 'brown-glass':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling brown glass'
    ...
    if class_names[np.argmax(predictions)] == 'green-glass':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling green glass'
    ...
    if class_names[np.argmax(predictions)] == 'white-glass':
        ...
        st.markdown(pdf_download_link, unsafe_allow_html=True)
        if st.button("Generate Video"):
            title='DIY ideas for reusing and recycling white glass'
